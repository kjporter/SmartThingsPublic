 ...
 */
definition(
    name: "Button Runs Routine",
    namespace: "bravenel",
    author: "Bruce Ravenel",
    description: "A momentary button causes a Routine to run",
    category: "My Apps",
...
def selections() {
	dynamicPage(name: "selections", title: "Select your button and routine", uninstall: true, install: true) {
		def phrases = location.helloHome?.getPhrases()*.label
		section() {
			input "myButton", "capability.button", title: "Button", required: true
			if(phrases) input "myPhrase", "enum", title: "Routine", required: true, options: phrases.sort()
...
def initialize() {
	subscribe(myButton, "switch.on", buttonHandler)
}

def buttonHandler(evt) {
	if(allOk) location.helloHome.execute(myPhrase)
}
